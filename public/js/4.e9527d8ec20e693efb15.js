...
function Edit(props) {
  ...
  var updateUser = function updateUser(event) {
    ...
    setLoading(true);
    inertia_react__WEBPACK_IMPORTED_MODULE_1__["Inertia"].patch("/users/".concat(props.user.id), {
      name: name,
      email: email
    }).then(function () {
      setLoading(false);
  ...
  var deleteUser = function deleteUser(event) {
    ...
    if (confirm('Are you sure you want to delete this contact?')) {
      inertia_react__WEBPACK_IMPORTED_MODULE_1__["Inertia"]["delete"]("/users/".concat(props.user.id)).then(function () {});
    }
  };
